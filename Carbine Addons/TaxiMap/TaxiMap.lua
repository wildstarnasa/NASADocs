		...
		for key, tObject in pairs(tObjects) do
			...
			if tTaxi.bUnlocked then
				...
				if tPath ~= nil then
					...
					if tPath.tPriceInfo.nAmount1 > GameLib.GetPlayerCurrency():GetAmount() then
						strTooltipString = strTooltipString .. string.format("<P Font=\"%s\" TextColor=\"%s\">%s</P>", "CRB_InterfaceSmall", "UI_WindowTextRed", Apollo.GetString("GenericError_Mail_InsufficientFunds"))
					end
				end
			else
