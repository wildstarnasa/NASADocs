...
function RaidFrameMasterLoot:Initialize(bShow)
	...
	self.wndMain = Apollo.LoadForm("RaidFrameMasterLoot.xml", "RaidFrameMasterLootForm", nil, self)
...
function RaidFrameMasterLoot:FactoryProduce(wndParent, strFormName, tObject)
	...
	if not wndNew then
		wndNew = Apollo.LoadForm("RaidFrameMasterLoot.xml", strFormName, wndParent, self)
