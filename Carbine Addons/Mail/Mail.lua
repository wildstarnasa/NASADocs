...
function Mail:OnMailResult(eResult)
	...
	local strErrorHeader = Apollo.GetString("CRB_Mail_ErrorGeneric")

	if tMailResultError[eResult] then
		if tMailHeaderError[eResult] then
			strErrorHeader = Apollo.GetString(tMailHeaderError[eResult])
		end
		strErrorMessage = Apollo.GetString(tMailResultError[eResult])

		if self.wndErrorMsg == nil then
			self.wndErrorMsg = Apollo.LoadForm(self.xmlDoc, "ErrorMessage", nil, self)
		end
		self.wndErrorMsg:FindChild("ErrorMessageBody"):SetText(strErrorMessage)
		self.wndErrorMsg:FindChild("ErrorMessageHeader"):SetText(strErrorHeader)
		self.wndErrorMsg:Show(true)
		self.wndErrorMsg:ToFront()
	else 
		return 
	end
...
function MailReceived:UpdateControls()
	...
	if not bHasAttachments then -- remove attachment panel
		...
		self.wndMain:FindChild("MessageText"):SetAnchorOffsets(nLeftMessage, nTopMessage, nRightMessage, -2)
	end
end
